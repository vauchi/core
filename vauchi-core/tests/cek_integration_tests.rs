...
fn test_process_cek_wrapped_update_saves_cek() {
    let (alice, bob_id, bob_identity, bob_dh, shared_secret) =
        setup_alice_with_bob_ratchet();
...
fn test_process_update_from_revoked_sender_rejected() {
    ...
    assert!(result.is_err(), "Updates from revoked senders should be rejected");
}
...
fn test_migrate_contacts_queues_updates() {
    ...
    let pending = alice
        ...
        .unwrap();
    assert_eq!(pending.len(), 1, "Migration should queue an update for each contact");
