...
//!
//! Run with: cargo bench -p webbook-core
...
fn bench_symmetric_encryption(c: &mut Criterion) {
    use webbook_core::crypto::{decrypt, encrypt, SymmetricKey};
...
fn bench_x3dh(c: &mut Criterion) {
    use webbook_core::exchange::{X3DHKeyPair, X3DH};
