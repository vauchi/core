...
//! Consent domain persistence view (consent_records/audit_log/deletion_state).
//!
...
/// Scoped persistence view for the consent domain.
...
impl Storage {
    /// Scoped persistence view for the consent domain.
    pub fn consent(&self) -> ConsentStore<'_> {
        ConsentStore {
            conn: &self.conn,
...
impl ConsentStore<'_> {
    ...
    pub fn save_deletion_state(
        &self,
        state: &crate::storage::DeletionState,
    ) -> Result<(), StorageError> {
        let json =
    ...
    pub fn load_deletion_state(&self) -> Result<crate::storage::DeletionState, StorageError> {
        let result = self.conn.query_row(
        ...
        match result {
            ...
            Ok(_) => Ok(crate::storage::DeletionState::None),
            Err(rusqlite::Error::QueryReturnedNoRows) => Ok(crate::storage::DeletionState::None),
            Err(e) => Err(StorageError::Database(e)),
        }
    }
    ...
    pub fn log_audit_event(
        ...
        let details_encrypted = if let Some(d) = details {
            Some(
                crate::crypto::encrypt(self.key, d.as_bytes())
                    .map_err(|e| StorageError::Encryption(e.to_string()))?,
            )
        } else {
            None
