...
//! `SyncManager` offline queue, incoming card-update processing, the
//! device-sync orchestrator, and binary-send helpers. These depend on
//! `crate::storage`; the pure sync domain (deltas, merkle, replay
//! detection, device-sync types) lives in `crate::sync`.
