...
//! Local-group domain persistence view.
//!
//! `GroupStore` owns the `local_groups` table (migration v35). Group names are
//! plaintext (user-visible labels with no security significance), so this store
//! needs no encryption key. Part of problem record
...
/// Scoped persistence view for local organization groups.
...
impl Storage {
    /// Scoped persistence view for the local-group domain.
    pub fn groups(&self) -> GroupStore<'_> {
        GroupStore {
            conn: &self.conn,
            clock: &self.clock,
        }
