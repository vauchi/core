...
fn insert_and_query_activity_log_entry() {
    ...
    let inserted = storage.activity_log_insert(&row).unwrap();
    assert!(inserted, "first insert should return true");

    let results = storage.activity_log_query_recent(now, 3600).unwrap();
...
fn duplicate_event_key_is_ignored() {
    ...
    let first = storage.activity_log_insert(&row).unwrap();
    assert!(first, "first insert should return true");

    let second = storage.activity_log_insert(&row).unwrap();
    assert!(
        !second,
...
fn prune_removes_old_entries() {
    ...
    storage.activity_log_insert(&old_row).unwrap();
    ...
    storage.activity_log_insert(&recent_row).unwrap();
