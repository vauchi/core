...
//! Pending domain persistence view (pending_updates).
//!
...
use super::super::error::{PendingUpdate, UpdateStatus};
use super::super::{Storage, StorageError};

/// Scoped persistence view for the pending domain.
...
impl PendingStore<'_> {
    ...
    pub fn queue_update(&self, update: &PendingUpdate) -> Result<(), StorageError> {
        ...
        let payload_encrypted = crate::crypto::encrypt(self.key, &update.payload)
            .map_err(|e| StorageError::Encryption(e.to_string()))?;
