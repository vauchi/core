...
//
// Migrated from `core/vauchi-platform/tests/it/app_preferences_tests.rs`
// as part of Phase 2a `mobile_app_preferences.rs` retirement
// (problem record
// `_private/docs/problems/2026-05-18-phase-2a-mobile-app-preferences-retirement/`).
//
// The four round-trip / overwrite / follow-system tests in that
// file are redundant with `vauchi-core/tests/it/app_preferences_storage_tests.rs`
// (storage layer is authoritative). Only the **pre-identity**
// invariant — that the singleton row is readable / writable before
// onboarding completes — carries wrapper-specific contract that the
// storage layer's `Storage::in_memory` setup does not exercise.
// That invariant moves here, against the PAE dispatch surface.

// @internal
#[test]
fn get_app_preferences_default_when_unset_via_dispatch() {
    let (engine, _dir) = create_engine_with_identity();
    match engine
        .dispatch_domain_command(DomainCommand::GetAppPreferences)
        .expect("get default prefs")
    {
        DomainCommandResult::AppPreferences { prefs } => {
...
// @internal
#[test]
fn set_then_get_app_preferences_round_trip_via_dispatch() {
    let (engine, _dir) = create_engine_with_identity();
    ...
    engine
        .dispatch_domain_command(DomainCommand::SetAppPreferences {
            prefs: written.clone(),
        })
        .expect("set prefs");
    ...
    match engine
        ...
        .expect("get prefs")
    {
        DomainCommandResult::AppPreferences { prefs: loaded } => {
...
// @internal
#[test]
fn app_preferences_dispatch_works_pre_identity() {
    // The Settings screen is reachable from the More tab before
    // onboarding completes; the singleton row must therefore be
    // readable / writable from the moment `PlatformAppEngine` is
    // constructed, with no identity on disk. Migrated from the
    // wrapper's `app_preferences_persist_without_identity` test
    // (the only wrapper-specific invariant not covered at the
    // storage layer).
    let (engine, _dir) = create_engine_no_identity();
    assert!(
        !engine.has_identity().expect("has_identity"),
        "fixture precondition: no identity yet"
    );
    ...
    engine
        ...
        .expect("set prefs pre-identity");

    match engine
        .dispatch_domain_command(DomainCommand::GetAppPreferences)
        .expect("get prefs pre-identity")
    {
        DomainCommandResult::AppPreferences { prefs: loaded } => {
