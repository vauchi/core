...
fn test_consent_upsert_and_check() {
    ...
    let granted = storage.consent().check_consent("analytics").unwrap();
    assert!(granted);
}
...
fn test_consent_check_nonexistent() {
    let storage = test_storage();
    let granted = storage.consent().check_consent("nonexistent").unwrap();
    assert!(!granted);
}
...
fn test_list_consent_records() {
    ...
    let records = storage.consent().list_consent_records().unwrap();
