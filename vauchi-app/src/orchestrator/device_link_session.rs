...
//! Plain-Rust device-link session orchestrator.
//!
//! Owns the relay-poll loop, the QR-expiry deadline, and the
//! user-confirm gate for the device-link protocol. Frontends connect
//! through their respective binding crate's listener adapter
//! (`vauchi-platform` for UniFFI, `vauchi-cabi` for C-callback) — the
//! state machine and cycle thread live here.
//!
//! # Lifecycle
//!
//! ```text
//! let session = vauchi_app::orchestrator::DeviceLinkSession::with_persistence_initiator(
//!     initiator, transport, identity_id, relay_timeout_secs,
...
//! exercised in vauchi-platform's `lib.rs` unit tests). Plan:
...
use crate::orchestrator::device_link_relay::{
...
///
/// Binding crates implement this trait via an adapter (UniFFI
/// `callback_interface` in `vauchi-platform`, C-callback function
/// pointer struct in `vauchi-cabi`) and register it with
/// [`DeviceLinkSession::set_listener`] before calling
/// [`DeviceLinkSession::start`]. Once `start()` is called, the cycle
/// thread drives the relay-poll loop and user-confirm gate, invoking
/// these callbacks as state advances.
///
/// # Threading
///
...
///    user to call [`confirm_manual`](DeviceLinkSession::confirm_manual)
///    / [`confirm_ultrasonic`](DeviceLinkSession::confirm_ultrasonic)
///    / [`deny`](DeviceLinkSession::deny).
...
/// Errors returned synchronously from session action methods.
///
/// Note: terminal session errors (relay timeout, deny, expiry) flow
/// through the listener's [`on_failed`](DeviceLinkSessionListener::on_failed)
/// callback as stable string reasons, not through this enum. This
/// type only captures argument-validation errors that surface to
/// the caller of [`DeviceLinkSession::confirm_manual`] /
/// [`DeviceLinkSession::confirm_ultrasonic`] before the cycle thread
/// observes the action.
#[derive(Error, Debug)]
pub enum DeviceLinkSessionError {
    /// Argument validation failed. The associated string identifies
    /// which argument and why; binding crates may surface it as-is
    /// or map it to a binding-specific error type.
...
/// User-action message sent from the binding thread into the cycle
...
impl DeviceLinkSession {
    ...
    pub fn confirm_ultrasonic(
        ...
        if challenge_response.len() != 16 {
            return Err(DeviceLinkSessionError::InvalidInput(
...
impl DeviceLinkSession {
    /// Production constructor — used by binding crates'
    /// `create_device_link_session_initiator` factories. Holds the
    /// persistence context so the cycle thread can save the updated
    /// device registry on successful confirm.
    pub fn with_persistence_initiator(
    ...
    /// `HttpTransportConfig::for_testing`), no persistence.
    #[doc(hidden)]
    ...
    /// per iteration. `0` restores production behaviour.
    #[doc(hidden)]
