...
fn test_unsupported_audio_device_returns_unknown_confidence() {
    ...
    let bob_card = ContactCard::new("Bob");
    // AU-5: Create verifier with AudioCapability::None — device can't do proximity
    ...
    // Proximity check was auto-invoked; device doesn't support audio
    // -> should be Unknown (not Low), because this is "can't check" not "checked and failed"
    ...
    match bob_session.state() {
        ExchangeState::Complete { contact } => {
            assert_eq!(
                *contact.proximity_confidence(),
                ProximityConfidence::Unknown,
                "Unsupported audio device should yield Unknown confidence, not Low"
            );
        }
        other => panic!("Expected Complete state, got {:?}", other),
    }
...
fn test_unsupported_device_explicit_proximity_check_returns_unknown() {
    ...
    // Explicitly run proximity check
    bob_session.run_proximity_check();
    ...
    match bob_session.state() {
        ExchangeState::Complete { contact } => {
            assert_eq!(
                *contact.proximity_confidence(),
                ProximityConfidence::Unknown,
                "Unsupported device should yield Unknown, not Low"
            );
        }
        other => panic!("Expected Complete state, got {:?}", other),
    }
