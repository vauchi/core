...
use serde::Serialize;
use webbook_core::contact_card::{is_allowed_scheme, ContactAction, ContactField, FieldType};
...
///
/// Uses webbook-core's URI builder for security validation before opening.
#[tauri::command]
pub async fn open_contact_field(
    ...
    // Get the action and URI using webbook-core's secure URI builder
    let action = field.to_action();
    let uri = field.to_uri();
