...
fn test_visibility_control_happy_path() {
    ...
    let bob_contact = Contact::from_exchange(bob_public_key, ContactCard::new("Bob"), SymmetricKey::generate());
    let bob_id = bob_contact.id().to_string();

    let carol_contact = Contact::from_exchange(carol_public_key, ContactCard::new("Carol"), SymmetricKey::generate());
    ...
    assert!(bob_rules.can_see("work", &bob_id), "Bob should see work phone");
    assert!(!bob_rules.can_see("personal", &bob_id), "Bob should NOT see personal phone");

    assert!(carol_rules.can_see("work", &carol_id), "Carol should see work phone");
    assert!(carol_rules.can_see("personal", &carol_id), "Carol should see personal phone");
    ...
    let bob_contact = alice_wb.get_contact(&bob_id).unwrap().unwrap();
    let bob_rules = bob_contact.visibility_rules();
    assert!(bob_rules.can_see("personal", &bob_id), "Bob should now see personal phone");
}
