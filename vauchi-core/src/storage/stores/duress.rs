...
//! Duress domain persistence view (duress_settings).
//!
...
/// Scoped persistence view for the duress domain.
pub struct DuressStore<'a> {
    conn: &'a Connection,
    key: &'a SymmetricKey,
...
impl Storage {
    /// Scoped persistence view for the duress domain.
    pub fn duress(&self) -> DuressStore<'_> {
        DuressStore {
            conn: &self.conn,
...
impl DuressStore<'_> {
    ...
    pub fn save_duress_settings(&self, settings: &DuressSettings) -> Result<(), StorageError> {
        ...
        let message_encrypted = crate::crypto::encrypt(self.key, settings.alert_message.as_bytes())
            .map_err(|e| StorageError::Encryption(e.to_string()))?;
