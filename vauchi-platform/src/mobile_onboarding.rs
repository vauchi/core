...
/// Helper for the legacy `mobile_onboarding.rs` surface (Phase 3
/// retirement target). `VauchiPlatform` does not hold a [`Clock`]
/// today, so timestamps come from the OS wall-clock here. Once this
/// file is retired (Phase 3), this helper goes with it.
fn now_secs() -> u64 {
...
impl VauchiPlatform {
    ...
    pub fn get_onboarding_progress(&self) -> Result<MobileOnboardingProgress, MobileError> {
        ...
        let progress = storage
            .load_or_create_onboarding_progress(now_secs())
