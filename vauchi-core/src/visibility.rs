...
//! Field-visibility domain types.
//!
//! Which contacts may see which fields on a contact card. This lives in a
//! neutral leaf module — depended on by `contact`, `contact_card`, and
//! `backup`, depending on none of them — so that a shared type used by
//! both `contact` and `contact_card` cannot reintroduce a
//! `contact ↔ contact_card` cycle (the reason it was never placed in
//! either).
...
pub struct VisibilityRules {
    ...
    /// `pub(crate)` so `contact_card` can read the raw map for serialization.
    pub(crate) rules: HashMap<String, FieldVisibility>,
}
