...
//! Retry domain persistence view (retry_entries).
//!
...
use super::super::error::RetryEntry;
use super::super::{Storage, StorageError};

/// Scoped persistence view for the retries domain.
pub struct RetryStore<'a> {
    conn: &'a Connection,
    key: &'a SymmetricKey,
}
...
impl Storage {
    /// Scoped persistence view for the retries domain.
    pub fn retries(&self) -> RetryStore<'_> {
        RetryStore {
            conn: &self.conn,
...
impl RetryStore<'_> {
    ...
    pub fn create_retry_entry(&self, entry: &RetryEntry) -> Result<(), StorageError> {
        let payload_encrypted = crate::crypto::encrypt(self.key, &entry.payload)
            .map_err(|e| StorageError::Encryption(e.to_string()))?;
