# WebBook Relay Deployment Guide

This guide covers deploying the WebBook Relay server in production.
...
sudo cp deploy/nginx/webbook-relay.conf /etc/nginx/sites-available/
sudo ln -s /etc/nginx/sites-available/webbook-relay.conf /etc/nginx/sites-enabled/

# Get certificates with certbot
sudo certbot --nginx -d relay.webbook.example.com
...
# Or manually:
sudo cp target/release/webbook-relay /usr/local/bin/
sudo cp deploy/systemd/webbook-relay.service /etc/systemd/system/
sudo useradd -r -s /bin/false webbook
sudo mkdir -p /var/lib/webbook-relay
sudo chown webbook:webbook /var/lib/webbook-relay
sudo systemctl daemon-reload
sudo systemctl enable --now webbook-relay
```
...
# helm repo add webbook https://charts.webbook.example.com

# Or install from local chart
helm install webbook-relay deploy/helm/webbook-relay \
  --set ingress.enabled=true \
  --set ingress.hosts[0].host=relay.webbook.example.com \
  --set ingress.tls[0].secretName=webbook-relay-tls \
  --set ingress.tls[0].hosts[0]=relay.webbook.example.com
```
...
The Docker image runs as non-root user `webbook` (UID 1000).
...
```bash
# Docker
docker logs webbook-relay

# Systemd
journalctl -u webbook-relay -f

# Kubernetes
kubectl logs -l app=webbook-relay -f
```
...
- Ensure the webbook user owns the data directory
- `chown -R webbook:webbook /var/lib/webbook-relay`
