...
//! Identity Deletion
//!
//! Provides identity deletion with secure data wipe and GDPR deletion grace period.
//! Vauchi has no "accounts" — the device IS the identity. This module handles
//! the self-sovereign act of destroying your own cryptographic identity.
...
/// Deletes all local identity data with secure overwrite.
///
...
pub fn delete_identity_data<P: AsRef<Path>>(db_path: P) -> Result<(), DeletionError> {
...
/// Result from executing identity deletion.
///
...
impl<'a> DeletionManager<'a> {
    ...
    /// `delete_identity_data()` to remove the database files.
    ///
