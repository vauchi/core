...
fn test_sync_update_propagation_happy_path() {
    ...
    alice_wb
        .add_own_field(ContactField::new(FieldType::Email, "work", "alice@old-company.com"))
        .unwrap();
    ...
    let bob_contact = Contact::from_exchange(bob_public_key, ContactCard::new("Bob"), shared_secret.clone());
    alice_wb.add_contact(bob_contact).unwrap();

    let alice_contact = Contact::from_exchange(alice_public_key, alice_wb.own_card().unwrap().unwrap(), shared_secret.clone());
    let alice_contact_id = alice_contact.id().to_string();
    bob_wb.add_contact(alice_contact).unwrap();

    // Initialize ratchets
    let bob_dh = X3DHKeyPair::generate();
    let mut alice_ratchet = DoubleRatchetState::initialize_initiator(&shared_secret, *bob_dh.public_key());
    ...
    modified_card.update_field_value(&email_field_id, "alice@new-company.com").unwrap();
    alice_wb.update_own_card(&modified_card).unwrap();

    alice_wb
        .add_own_field(ContactField::new(FieldType::Phone, "mobile", "+15559999999"))
        .unwrap();
    ...
    let email_field = updated_card.fields().iter().find(|f| f.label() == "work").unwrap();
    assert_eq!(email_field.value(), "alice@new-company.com");

    let phone_field = updated_card.fields().iter().find(|f| f.label() == "mobile").unwrap();
...
fn test_sync_manager_queue_happy_path() {
    ...
    old_card.add_field(ContactField::new(FieldType::Email, "work", "alice@old.com")).unwrap();

    let mut new_card = ContactCard::new("Alice");
    new_card.add_field(ContactField::new(FieldType::Email, "work", "alice@new.com")).unwrap();

    // Queue update for offline contact
    let contact_id = "bob-123";
    let update_id = sync_manager.queue_card_update(contact_id, &old_card, &new_card).unwrap();
    ...
    assert!(matches!(state, webbook_core::SyncState::Pending { queued_count: 1, .. }));
...
fn test_full_three_user_workflow() {
    let mut alice_wb: WebBook<MockTransport> = WebBook::in_memory().unwrap();
    let mut bob_wb: WebBook<MockTransport> = WebBook::in_memory().unwrap();
    let mut carol_wb: WebBook<MockTransport> = WebBook::in_memory().unwrap();
    ...
    alice_wb.add_own_field(ContactField::new(FieldType::Email, "personal", "alice@home.com")).unwrap();
    alice_wb.add_own_field(ContactField::new(FieldType::Phone, "work", "+15551111111")).unwrap();
    alice_wb.add_own_field(ContactField::new(FieldType::Phone, "personal", "+15552222222")).unwrap();
    ...
    let bob_contact = Contact::from_exchange(bob_pk, ContactCard::new("Bob"), alice_bob_secret.clone());
    ...
    let carol_contact = Contact::from_exchange(carol_pk, ContactCard::new("Carol"), alice_carol_secret.clone());
    ...
    assert!(carol_contact.visibility_rules().can_see("personal", &carol_id));
    ...
    let mut modified_card = old_card.clone();
    modified_card.update_field_value(&work_email_id, "alice@new-company.com").unwrap();
    alice_wb.update_own_card(&modified_card).unwrap();
    ...
    let mut bob_alice_card = bob_wb.get_contact(&alice_id_bob).unwrap().unwrap().card().clone();
    delta.apply(&mut bob_alice_card).unwrap();

    let mut carol_alice_card = carol_wb.get_contact(&alice_id_carol).unwrap().unwrap().card().clone();
...
fn test_relay_update_delivery_happy_path() {
    ...
    let msg_id = client
        .send_update("bob-recipient-id", &mut alice_ratchet, update_payload, "update-001")
        .unwrap();

    // Verify update is tracked
    assert!(!msg_id.is_empty());
    assert_eq!(client.in_flight_count(), 1);
    assert!(client.in_flight_update_ids().contains(&"update-001".to_string()));
