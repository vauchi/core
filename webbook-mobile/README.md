...
UniFFI bindings for WebBook - exposes the Rust core library to Android and iOS.

## Purpose

This crate wraps `webbook-core` with a mobile-friendly API via UniFFI. It provides:
...
cargo run -p webbook-mobile --bin uniffi-bindgen generate \
    --library target/release/libwebbook_mobile.so \
...
val webbook = WebBookMobile("path/to/data", "ws://relay.example.com:8080")

// Create identity
webbook.createIdentity("Alice")

// Add contact fields
webbook.addField(MobileFieldType.EMAIL, "work", "alice@company.com")
webbook.addField(MobileFieldType.PHONE, "mobile", "+1-555-1234")

// Exchange contacts
val qrData = webbook.generateExchangeQr()
// Show QR code to other user...

// Complete exchange from scanned QR
val result = webbook.completeExchange(scannedQrData)

// List contacts
val contacts = webbook.listContacts()
```
...
| Type | Description |
|------|-------------|
| `WebBookMobile` | Main interface object |
