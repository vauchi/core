...
//! DeviceDelivery domain persistence view (device_deliveries).
//!
//! Part of problem record `2026-06-09-storage-per-domain-store-boundaries` (Phase 1).

use rusqlite::{Connection, params};

use super::super::error::{DeliverySummary, DeviceDeliveryRecord, DeviceDeliveryStatus};
use super::super::{Storage, StorageError};

/// Scoped persistence view for the device_deliveries domain.
pub struct DeviceDeliveryStore<'a> {
    conn: &'a Connection,
}

impl Storage {
    /// Scoped persistence view for the device_deliveries domain.
    pub fn device_deliveries(&self) -> DeviceDeliveryStore<'_> {
        DeviceDeliveryStore { conn: &self.conn }
    }
}

impl DeviceDeliveryStore<'_> {
