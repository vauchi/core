...
### `api/` - High-Level API
The main entry point for applications using WebBook.

| File | Purpose |
|------|---------|
| `webbook.rs` | Main `WebBook` orchestrator and builder |
| `config.rs` | Configuration types (`WebBookConfig`, `RelayConfig`) |
| `contact_manager.rs` | Contact CRUD operations |
| `sync_controller.rs` | Sync + network coordination |
| `events.rs` | Event system (`WebBookEvent`, handlers) |
| `error.rs` | Unified `WebBookError` type |
