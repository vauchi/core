...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vauchi_app_device_link_peer_connected(
    handle: *mut VauchiApp,
    verification_code: *const c_char,
) -> *mut c_char {
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if handle.is_null() {
            return std::ptr::null_mut();
        }
        let code = match from_c_str(verification_code) {
            Some(s) => s,
            None => return std::ptr::null_mut(),
        };
        // SAFETY: handle is checked non-null above
        let app = unsafe { &*handle };
        match app.engine.lock() {
            Ok(mut engine) => match engine.device_link_peer_connected(code) {
                Some(screen) => serde_json::to_string(&screen).map_or_else(
                    |e| to_c_string(&format!(r#"{{"error":"{}"}}"#, e)),
                    |j| to_c_string(&j),
                ),
...
/// `handle` must be a valid app handle or null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn vauchi_app_device_link_sync_complete(
    handle: *mut VauchiApp,
) -> *mut c_char {
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if handle.is_null() {
        ...
        let app = unsafe { &*handle };
        match app.engine.lock() {
            Ok(mut engine) => match engine.device_link_sync_complete() {
                Some(screen) => serde_json::to_string(&screen).map_or_else(
                    |e| to_c_string(&format!(r#"{{"error":"{}"}}"#, e)),
                    |j| to_c_string(&j),
                ),
