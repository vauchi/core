...
///
/// Provides spacing, typography, border radius, touch target, motion,
/// and avatar-fallback values that all platform clients use for
...
/// Avatar rendering constants. `fallback_bg` is the `#rrggbb` background
/// drawn behind initials when a contact carries no `bg_color` of its own
/// (RG-11: it was a Windows-only hardcode until the token existed).
#[cfg_attr(feature = "schema-gen", derive(schemars::JsonSchema))]
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
