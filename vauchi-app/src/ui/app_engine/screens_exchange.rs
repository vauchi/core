    ...
    pub(super) fn create_exchange_engine(
        ...
        match screen {
            ...
            AppScreen::MultiStageExchange { mode } => {
                // Crypto runs in the AppEngine-owned
                // `MultiStageMachine` (fed via
                // `forward_multi_stage_hardware_event` /
                // `apply_multi_stage_event`); this engine is the pure
                // rendering projection. The cycle-thread
                // `MobileMultiStageSession` bridge it replaced is
                // retired.
                //
                // Mode-aware constructor (Phase 1.E of
                // `2026-05-11-hover-graduation-plan.md`): Hover gets
                // `new_hover()` (front camera + audio-handshake
                // trigger); Glance gets `new_glance()` (back camera +
                // audio-quiet), gated on
                // `is_active_engine_multi_stage_hover()` so Glance
                // flows never fire spurious audio chrome.
